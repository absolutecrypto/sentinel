...
class AbsoluteDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some absoluted methods
    ...
    @classmethod
    def from_absolute_conf(self, absolute_dot_conf):
        from absolute_config import AbsoluteConfig
        config_text = AbsoluteConfig.slurp_config_file(absolute_dot_conf)
        creds = AbsoluteConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = absolutelib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = absolutelib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = absolutelib.blocks_to_seconds(diff)
