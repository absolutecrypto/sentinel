...
class AbsoluteConfig():
    ...
    def slurp_config_file(self, filename):
        # read absolute.conf config but skip commented lines
        f = io.open(filename)
        lines = []
        for line in f:
        ...
        # data is absolute.conf without commented lines
        data = ''.join(lines)
    ...
    def get_rpc_creds(self, data, network='mainnet'):
        # get rpc info from absolute.conf
        match = re.findall(r'rpc(user|password|port)=(.*?)$', data, re.MULTILINE)

        # python >= 2.7
        creds = {key: value for (key, value) in match}

        # standard Absolute defaults...
        default_port = 18889 if (network == 'mainnet') else 17778

        # use default port for network if not specified in absolute.conf
        if not ('port' in creds):
            creds[u'port'] = default_port

        # convert to an int if taken from absolute.conf
