...
def is_valid_absolute_address(address, network='mainnet'):
    ...
    absolute_version = 140 if network == 'testnet' else 23
    ...
    try:
        ...
        # rescue from exception, not a valid Absolute address
        return False

    if (address_version != absolute_version):
        return False
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('absolute.conf')
