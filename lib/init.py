...
def has_absolute_conf():
    import config
    import io

    valid_absolute_conf = False

    # ensure absolute_conf exists & readable
    #
    # if not, print a message stating that Absolute Core must be installed and
    # configured, including JSONRPC access in absolute.conf
    try:
        f = io.open(config.absolute_conf)
        valid_absolute_conf = True
    except IOError as e:
        print(e)

    return valid_absolute_conf
...
def main():
    ...
    if not has_absolute_conf():
        print("AbsoluteCore must be installed and configured, including JSONRPC access in absolute.conf")
        sys.exit(1)
