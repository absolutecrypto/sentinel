...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, absoluted):
        golist = absoluted.rpc_command('gobject', 'list')
    ...
    def import_gobject_from_absoluted(self, absoluted, rec):
        ...
        # exclude any invalid model data from absoluted...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from absoluted, with every run
        ...
        # get/create, then sync payment amounts, etc. from absoluted - Absoluted is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(absoluted)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from absoluted! %s" % e)
            govobj.vote_delete(absoluted)
            return (govobj, None)
    ...
    def vote_delete(self, absoluted):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(absoluted, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, absoluted, signal, outcome):
        import absolutelib
        ...
        output = absoluted.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = absolutelib.did_we_vote(output)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    def is_valid(self):
        ...
        try:
            ...
            if not absolutelib.is_valid_absolute_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Absolute address for network [%s], returning False" % (self.payment_address, config.network))
                return False
...
class Superblock(BaseModel, GovernanceClass):
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not absolutelib.is_valid_absolute_address(addr, config.network):
    ...
    def hash(self):
        import absolutelib
        return absolutelib.hashit(self.serialise())
