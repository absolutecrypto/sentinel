...
class GovernanceClass(object):
    ...
    def vote(self, absoluted, signal, outcome):
        return self.go.vote(absoluted, signal, outcome)

    # pass thru to GovernanceObject#voted_on
    def voted_on(self, **kwargs):
        return self.go.voted_on(**kwargs)

    def vote_validity(self, absoluted):
        if self.is_valid():
            printdbg("Voting valid! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(absoluted, models.VoteSignals.valid, models.VoteOutcomes.yes)
        else:
            printdbg("Voting INVALID! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(absoluted, models.VoteSignals.valid, models.VoteOutcomes.no)
